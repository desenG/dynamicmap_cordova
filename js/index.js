...
function intfunc() {
    ...
    //addHammerRecognizer(document.querySelector("[data-role=listview]"));
...
function onSuccess(contacts) {
    ...
    for (var i = 0; i < contacts.length; i++) {
        ...
        if (contacts[i].displayName == null) //for ios
        {
            person.name = contacts[i].name.givenName + ' ' + contacts[i].name.familyName;
        } else //for android
        {
            person.name = contacts[i].displayName;
        }
        person.phoneNumbers = contacts[i].phoneNumbers;
        person.emails = contacts[i].emails;
        person.addresses = contacts[i].addresses;
        person.birthday = contacts[i].birthday;
        ...
        li.setAttribute('data-ref', person.id);
        ...
        //alert("xxxx");
        //list += '<li data-ref="'+person.id+'">' + person.name + '</li>';
    }
